...
class GGPTMedalBuyer(_PluginBase):
    ...
    def run_once_api(self) -> Dict[str, Any]:
        if self._lock.locked():
            logger.warn("立即检查请求被忽略：已有 GGPT 勋章任务正在执行")
    ...
    def run_buy_task(self, force: bool = False) -> Dict[str, Any]:
        if not self._lock.acquire(blocking=False):
            logger.warn("GGPT 勋章购买任务启动失败：已有任务正在执行")
            return {"success": False, "message": "已有购买任务正在执行"}
    ...
    def __judge_purchase_result(self, text: str, status_code: int) -> Tuple[str, str]:
        ...
        try:
            ...
            logger.warn(f"GGPT 勋章购买接口返回非 JSON：响应预览={preview}")
            ...
            if user_id:
                try:
                    ...
                    if details_expire_at:
                        ...
                        return
                    logger.warn("GGPT 个人中心未解析到当前勋章到期时间，将使用兜底规则")
                except Exception as err:
                    logger.warn(f"读取 GGPT 个人中心到期时间失败：{err}")
            else:
                logger.warn("未能从 GGPT Cookie 解析 user_id，无法读取个人中心到期时间")
    ...
    def __parse_owned_expire_time(self, html: str, medal_name: str) -> Optional[datetime]:
        plain = self.__html_to_text(html)
        keywords = [word for word in [medal_name, self.DEFAULT_MEDAL_NAME, self._medal_id] if word]
        for keyword in keywords:
            idx = plain.find(str(keyword))
            if idx < 0:
                continue
            context = plain[max(0, idx - 300):idx + 500]
            if not any(mark in context for mark in ["到期", "过期", "有效期", "拥有", "我的勋章"]):
                continue
            times = re.findall(r"\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}", context)
            if times:
                try:
    ...
    def __parse_userdetails_expire_time(self, html: str, medal_info: Dict[str, Any]) -> Optional[datetime]:
        plain = self.__html_to_text(html)
        explicit_expire_at = self.__parse_expire_label_time(plain)
        if explicit_expire_at:
            return explicit_expire_at

        keywords = [
            word for word in [
                medal_info.get("name"),
                self.DEFAULT_MEDAL_NAME,
                self._medal_id
            ] if word
        ]
        now = datetime.now()
        for keyword in keywords:
            ...
            if idx < 0:
                continue
            context = plain[max(0, idx - 1000):idx + 1500]
            times = self.__extract_datetimes(context)
            future_times = [item for item in times if item > now - timedelta(minutes=5)]
            if future_times:
                return max(future_times)
            if times:
                return max(times)
        return None
    ...
    def __medal_html_context(self, html: str) -> str:
        ...
        row_pattern = rf"<tr[^>]*>.*?(?:data-id=[\"']?{medal_id}[\"']?|\b>{medal_id}</(?:td|th)>).*?</tr>"
        row_match = re.search(row_pattern, html, flags=re.IGNORECASE | re.DOTALL)
        if row_match:
            return row_match.group(0)
        id_match = re.search(rf"data-id=[\"']?{medal_id}[\"']?", html, flags=re.IGNORECASE)
        if id_match:
            start = max(0, id_match.start() - 1200)
            end = min(len(html), id_match.end() + 1200)
            return html[start:end]
        text_match = re.search(rf">\s*{medal_id}\s*<", html)
        if text_match:
            start = max(0, text_match.start() - 1200)
            end = min(len(html), text_match.end() + 1200)
            return html[start:end]
        return ""
